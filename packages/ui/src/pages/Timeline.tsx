...
export default function Timeline() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h2 className="text-xl font-semibold text-[var(--foreground)] mb-4">Timeline</h2>

      {/* Filters */}
      <div className="flex flex-wrap gap-3 mb-6 p-4 bg-[var(--card)] border border-[var(--border)] rounded-lg">
        <div className="flex-1 min-w-[150px]">
          <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1">Personnage</label>
          <select
            value={charFilter}
        ...
        <div className="flex-1 min-w-[150px]">
          <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1">Lieu</label>
          <select
            value={locFilter}
        ...
        <div className="min-w-[100px]">
          <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1">Chapitre de</label>
          <input
            type="text"
            value={chapterFrom}
        ...
        <div className="min-w-[100px]">
          <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1">a</label>
          <input
            type="text"
            value={chapterTo}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-[var(--muted-foreground)]">
          ...
          <button
            ...
            className="mt-3 text-[var(--sidebar-primary)] hover:underline text-sm"
          >
            Creer un evenement
          </button>
        </div>
          ...
          {Array.from(chapters.entries()).map(([chapter, chapterEvents]) => (
            <div key={chapter}>
              ...
              <div className="relative flex items-center mb-3 mt-6 first:mt-0">
                ...
                <span className="ml-11 text-xs font-semibold text-amber-400 uppercase tracking-wide bg-amber-500/10 px-2 py-0.5 rounded">
              ...
              {chapterEvents.map((ev, idx) => (
                ...
                >
                  {/* Dot */}
                  <div className="absolute left-2.5 mt-3 w-3 h-3 bg-[var(--sidebar-primary)] rounded-full border-2 border-[var(--background)] z-10" />

                  {/* Card */}
                  <button
                    onClick={() => navigate(`/events/${ev.id}`)}
                    className="ml-11 flex-1 text-left p-3 bg-[var(--card)] border border-[var(--border)] rounded-lg hover:shadow-md hover:border-[var(--border)] transition-all duration-200 cursor-pointer"
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-medium text-[var(--foreground)]">{ev.title}</span>
                      {ev.chapter && (
                    ...
                    {ev.locationName && (
                      <div className="flex items-center gap-1 mb-1">
                        <span className="text-xs text-emerald-400">{ev.locationName}</span>
                      </div>
                    )}
                    ...
                    {ev.characterDetails && ev.characterDetails.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {ev.characterDetails.map((c) => (
                          <span
                            key={c.id}
                            className="text-[10px] px-1.5 py-0.5 bg-blue-500/10 text-blue-400 rounded-full"
                          >
                            {c.name}
                          </span>
                        ))}
                  ...
                  <div className="flex flex-col ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {idx > 0 || Array.from(chapters.values()).indexOf(chapterEvents) > 0 ? (
                      <button
                        ...
                        className="text-[var(--muted-foreground)] hover:text-[var(--foreground)] p-0.5"
                        title="Monter"
                      >
